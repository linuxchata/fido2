...
public sealed class Fido2Configuration
{
    ...
    /// <summary>
    /// Gets a valid domain string identifying the Relying Party on whose behalf a given registration or
    /// authentication ceremony is being performed. This is a critical parameter in the WebAuthn protocol.
    /// It defines the security scope within which credentials are valid. Therefore, careful selection is
    /// essential, as an incorrect or overly broad value can lead to unintended credential reuse or security
    /// vulnerabilities.
    /// </summary>
    ...
    /// None attestation is used when the authenticator doesn't have any attestation information available.
    /// </summary>
    ...
    /// Self attestation is used when the authenticator doesn't have a dedicated attestation key pair or
    ...
    /// <summary>
    /// Gets or sets a value indicating whether the Relying Party uses the Metadata Service to verify
    ...
    /// If enabled, missing metadata for the authenticator would cause attestation to fail.
    /// </summary>
