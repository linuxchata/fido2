...
public class PackedAttestationStatementStategyTests
{
    ...
    {
        // Arrange
        var attestationObjectValidatorMock = new Mock<IAttestationObjectValidator>();
        attestationObjectValidatorMock
            .Setup(a => a.Validate(
        ...
        var handler = new AttestationObjectHandler(provider, attestationObjectValidatorMock.Object);

        var result = handler.Handle(attestationObject, clientData, creationOptions);

        // Act
        _sut.Validate(result.Value!, clientData, creationOptions);
    }
}
